...
<script>
    ...
    function initPaper() {
        ...
        $.ajax({
            type: "post",
            dataType: 'json',
            url: "/school/onlineExam/chapterTest/startCourseWorkPaper?paperId=" + paperId + "&&studentId=" + studentId + "&&paperRule=" + paperRule,
            ...
            success: function (data) {
                ...
                //     allowNum--;
                //     if(allowNum >= 0){
                //         alert("您已切屏,超过三次自动提交试卷(剩余"+allowNum+"次)");
                //     }else{
                //         alert("您已切屏超过三次,已自动提交试卷");
                //     }
                //
                // }
                //目录部分
                    ...
                    if (data[i].tQuestiontemplateNum == "5" && data[i].titleTypeNum == 10 && data[i].stuOptionExamanswerList == "") {
                        ...
                        for (var j = 0; j < ansList.length; j++) {
                            ...
                            $('#content').append(' <li style="list-style: none">' +
                                ...
                                '<form action="/school/onlineExam/testPaperOne/uploadImg?paperId=' + paperId + '&&studentId=' + studentId + '&&stoption=' + ansList[j].id + '" id="domeform' + ansList[j].id + '" target="frameName' + ansList[j].id + '" method="post" enctype="multipart/form-data">' +
...
// }
    ...
    function issubmitsave() {
        ...
        if (!statu) {
            ...
            $.ajax({
                type: "post",
                url: "/school/onlineExam/chapterTest/submitPaperWork",
                dataType: "json",
            ...
            <#--    dataType: "json",-->
            <#--    contentType: "application/json;charset=UTF-8",-->
            <#--    url: "/school/onlineExam/userExam/setState",-->
            <#--    data:-->
