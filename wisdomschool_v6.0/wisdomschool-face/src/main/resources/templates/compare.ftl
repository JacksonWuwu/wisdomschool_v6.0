						...
						<div class="col-md-2 h-100 p-2 nav flex-column justify-content-center bg-light text-center " style="border-width:1px 0 1px 0;border-style:solid;border-color:#40B6E0;">
							...
							<h4 class="text-center text-danger ">
								<div class="dmaku-container">
									<div class="circleChart" id="1" data-value="0" style="color:white"></div>
								</div>
							</h4>
						</div>
				...
				<div class="row w-100 h-25 m-0 mb-2 p-1" >
					<div class="col-md-12 text-center text-info nav flex-column justify-content-center mb-3" style="border: #40B6E0 1px solid;border-radius: 8px" >
						<div>
							...
							<span class="font-weight-bold h3" id="loginName">${loginName}</span>
						</div>
						<div>
							<label class="font-weight-bold h3">姓名:</label>
							<span class="font-weight-bold h3 " id="userName">${userName}</span>
						</div>
					</div>
				</div>
			</div>
...
<script type="text/javascript">
		...
		tracker.on('track', function(event) {
			if(!tipFlag){
				...
				if (event.data.length === 0) {
					...
					//检测到一张人脸
					if(!tipFlag){
						informationTitle.html('已检测到人脸，正在拍照，请勿乱动~');
						// 给检测到的人脸绘制矩形
	...
	function takePhoto() {
		...
		if(!$("#picture").is(":visible"))
		{
			...
			if (file == undefined) {
				$.modal.alertWarning("请选择有人脸的图片进行识别");
				return;
			}
	...
	function register(formData){
		...
		$.ajax({
			...
			success: function (res) {
				...
				if (res.code == 0) {
					isRegistered++;
					let number = parseInt(res.data.compare*100);

					$("#dbimg").attr("src", res.data.dbImage);
					$("#1").attr("data-value",number);
					$(".circleChart#1").circleChart({
						value: number,
					});
