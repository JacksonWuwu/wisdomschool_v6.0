...
<script>
    ...
    function initPaper() {
        $.ajax({
            type: "post",
            dataType: 'json',
            url: "/wx/exam/startCoursePaper?paperId=" + paperId+"&&studentId="+studentId+"&&paperRule="+paperRule+"&&openId="+openId,
                    ...
                    if (data[i].tQuestiontemplateNum == "6") {
                        ...
                        for (var j = 0; j < ansList.length; j++) {
                            ...
                            $('#content').append(' <li style="list-style: none">' +
                                    ...
                                    '"anwserchange(' + tQuestiontemplateNum + ',' + dataId + ',' + anlId6 + ')">&nbsp;提交答案&nbsp;</button> ' +
                                    '</div>'+
                                    '<textarea name="title" rows="5" id="' + ansList[j].id +'"'+
                                    'style="margin: 0px; width: 569px; height: 152px;"' +
                                    'onchange=' + '"anwserchange(' + tQuestiontemplateNum + ',' + dataId + ',' + anlId6 + ')">' + trueAns + ' ' +
                                    ' </textarea>' +
                                    '</div></div></li></ul></div></div>');
    ...
    function saveOrUpdateAnswer() {
        for (var i = 0; i < tqg; i++) {
            $.ajax({
                type: "post",
                url: "/wx/exam/saveCourseTestAnswer",
                data: {
