...
<script type="text/javascript">
    ...
    function loadAlreadyTable() {
        ...
        $('#already-table').bootstrapTable({
            url: '/school/onlineExam/myQuestion/paperQuestion/list2?paperId=' + ${testPaper.id},
            method: 'post',
            dataType:'json',
            sortOrder: "desc",
            cache: false,                        //是否使用缓存
    ...
    function saveSelectedQuestions(paperId, id) {
        //  通知俩个table更新
        $.modal.open('添加试题', "/school/paper/question/addTwo?paperId="+ paperId + "&&myQuestionId=" + id, function () {
    ...
    function removeAlready(id) {
        $.modal.confirm("确认要删除?", function () {
            ...
            $.operate.submit("/school/onlineExam/myQuestion/paperQuestion/remove2", "post", "json", data, true, function (result) {
    ...
    function submitHandler() {
        $.ajax({
            type: "POST",
            url: "/school/onlineExam/testPaperWork/updateScore?paperId="+testid,
            dataType: 'json',
    ...
    function showPaper() {
        $.ajax({
            type: "POST",
            url: "/school/onlineExam/testPaperOne/buildTestToShow",
            dataType: 'json',
            contentType: "application/json;charset-UTF-8",
