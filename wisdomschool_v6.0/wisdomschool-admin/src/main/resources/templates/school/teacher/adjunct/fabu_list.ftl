...
<script type="text/javascript">
        ...
        let options = {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    formatter: function (value, row, index) {
                        let actions = [];
                        let clbumName=row.clbumName;
                        <#--                        <@shiro.hasPermission name="jiaowu:grades:edit">-->
                        // actions.push('<a class="btn btn-success btn-xs " href="#" onclick="$.operate.edit(' + row.id + ')"><i class="fa fa-edit"></i>重新发布</a> ');
                        <#--  school/onlineExam/testPaperOne                      </@shiro.hasPermission>-->
                        <#--                        <@shiro.hasPermission name="jiaowu:grades:remove">-->
                        ...
                        <#--                        </@shiro.hasPermission>-->
                        actions.push('<a class="btn btn-success btn-xs" href="#" onclick="refabu('+row.id+','+'&quot;'+clbumName+'&quot;'+')"><i class="fa fa-play"></i>重新发布</a>&nbsp;&nbsp;&nbsp;');
                        actions.push('<a class="btn btn-warning btn-xs " href="#" onclick="detailFull(' + row.id +')"><i class="fa fa-search"></i>详细</a>');
                        return actions.join('');
                    }
                }]
        };
    ...
    /*调度任务-启用*/
    function refabu(id,clbumName) {
        // var id = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
        $.post("/teacher/adjunct/addfabu",{
            "id": id,
            "clbums": clbumName,
        },function (data) {
            if(data.msg === "操作成功"){
