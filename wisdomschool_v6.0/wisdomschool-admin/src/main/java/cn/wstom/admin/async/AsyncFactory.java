...
import cn.wstom.admin.service.impl.SysLogininforServiceImpl;
import cn.wstom.admin.service.impl.SysUserOnlineServiceImpl;
import cn.wstom.admin.utils.IpUtil;
import cn.wstom.admin.utils.LogUtils;
import cn.wstom.admin.utils.ServletUtils;
import cn.wstom.common.constant.Constants;
import cn.wstom.common.utils.AddressUtils;

import cn.wstom.common.utils.IpUtils;
...
public class AsyncFactory {
    ...
    public static TimerTask syncSessionToDb(final OnlineSession session) {
        return new TimerTask() {
            ...
            public void run() {
                ...
                try {
                    SpringUtils.getBean(SysUserOnlineServiceImpl.class).save(online);
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    public static TimerTask recordLoginInfo(final String username, final String status, final String message, final Object... args) {
        final UserAgent userAgent = UserAgent.parseUserAgentString(ServletUtils.getRequest().getHeader("SysUser-Agent"));
        final String ip = IpUtil.getIpAddr(ServletUtils.getRequest());
        return new TimerTask() {
            @Override
            public void run() {
                ...
                if (Constants.LOGIN_SUCCESS.equals(status) || Constants.LOGOUT.equals(status)) {
                    ...
                    logininfor.setStatus(Constants.FAILURE);
                }
                // 插入数据
                try {
