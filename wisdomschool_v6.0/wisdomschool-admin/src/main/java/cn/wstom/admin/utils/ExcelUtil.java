...
import cn.wstom.admin.service.BaseService;
import cn.wstom.common.annotation.Excel;
import cn.wstom.common.annotation.Excel.Type;
import cn.wstom.common.base.Data;

import cn.wstom.common.constant.Constants;
import cn.wstom.common.reflection.ReflectionProvider;
import cn.wstom.common.reflection.SimpleReflectionProvider;
import cn.wstom.common.support.Convert;
import cn.wstom.common.utils.*;
...
public class ExcelUtil<T> {
    ...
    public List<T> importExcel(String sheetName, InputStream is) throws Exception {
        ...
        if (rows > 0) {
            ...
            Map<Integer, Field> fieldsMap = new HashMap<>(this.fields.size());
            ...
            ReflectionProvider rp = new SimpleReflectionProvider();
                ...
                for (int column = 0; column < serialNum; column++) {
                    ...
                    if (entity == null) {
                        ...
                        for (Field f : this.fields) {
                            if (!TypeUtils.isBaseType(f.getType())) {
                                f.setAccessible(true);
                                f.set(entity, f.getType().newInstance());
                            }
                        }
                    }

                    context.setRoot(entity);
                    // 从map中得到对应列的field.
                    ...
                    if (StringUtil.isNotNull(fieldType)) {
                        ...
                        if (StringUtils.isNotEmpty(attr.targetAttr())) {
                            propertyName = field.getName() + Constants.SEPARATOR_DOT + attr.targetAttr();
                        } else if (StringUtils.isNotEmpty(attr.readConverterExp())) {
                            val = reverseByExp(String.valueOf(val), attr.readConverterExp());
                        }
                        System.out.println(propertyName + " :: " + val);
                        rp.setValue(propertyName, context, context.getRoot(), val);
                    }
                }
                list.add(entity);
            }
            LOGGER.info("加载excel数据完成...");
        }
        return list;
    }
     ...
     * @return 结果
     */
    public Data exportExcel(List<T> list, String sheetName) throws Exception {
     ...
     * @return 结果
     */
    public Data importTemplateExcel(String sheetName) throws Exception {
     ...
     * @return 结果
     */
    public Data exportExcel() throws Exception {
        OutputStream out = null;
        try {
            // 取出一共有多少个sheet.
            double sheetNo = Math.ceil(1.0 * list.size() / SHEET_SIZE);
            for (int index = 0; index <= sheetNo; index++) {
                ...
                for (int i = 0; i < fields.size(); i++) {
                    ...
                    if (StringUtils.isNotEmpty(attr.prompt())) {
                        // 这里默认设了2-101列提示.
                        setHSSFPrompt(sheet, StringUtils.EMPTY, attr.prompt(), 1, 100, i, i);
                    }
                    ...
                    if (attr.comboField().length > 0) {
                        String beanName = field.getName() + "ServiceImpl";
                        BaseService baseService = SpringUtils.getBean(beanName);
                        List list = baseService.list(null);
                        ReflectionProvider rp = new SimpleReflectionProvider();
                        OgnlContext context = new OgnlContext();
                        List<String> name = new LinkedList<>();
                        //暂实现写入单个字段
                        String comboField = attr.comboField()[0];
                        list.forEach(l -> {
                            try {
                                context.setRoot(l);
                                name.add(String.valueOf(rp.getValue(comboField, context, context.getRoot())));
                                context.clear();
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
                        });
                        String[] names = name.toArray(new String[]{});

                        // 这里默认设了2-101列只能选择不能输入.
                        setHSSFValidation(sheet, names, 1, 100, i, i);
                    }
                }
    ...
    public String getAbsoluteFile(String filename) {
        String downloadPath = FileUtils.getTmpDir() + filename;
    ...
    private Object getTargetValue(T vo, Field field, Excel excel) throws Exception {
        ...
        if (StringUtils.isNotEmpty(excel.targetAttr())) {
            ...
            if (target.indexOf(Constants.SEPARATOR_DOT) > -1) {
