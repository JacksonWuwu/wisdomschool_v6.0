...
<#--                        </@shiro.hasPermission>-->
    ...
    function openwindow(idAndNum) {
        ...
        window.open('/school/onlineExam/testPaper/makeScore?id=' + id + '&&stuNum=' + stuNum, '_blank');
    }
    ...
    function showPaper(content) {
        ...
        var id=idAndStuNum[0];
        var stuNum =idAndStuNum[1];
        window.open('/school/onlineExam/frontExam/showPaper?id='+id+'&&stuNum='+stuNum,'_blank');
    }
    ...
    function removePaper(con){
        ...
        $.modal.confirm("确认要删除?", function () {
            ...
            $.operate.submit("/school/onlineExam/frontExam/studentPaper/remove", "post", "json", data, true, function (result) {
    ...
    function getTcoName(tgId) {
        ...
        $.ajax({
            type: 'POST',
            url: '/school/onlineExam/frontExam/getTcoName?cId=' + cId + "&&tid=" + tid+"&&tgId="+tgId,
            dataType: 'json',
            success: function (data) {
                for (i = 0; i < data.length; i++) {
