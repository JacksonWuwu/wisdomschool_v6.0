...
<div id="divpx2">
    ...
    <form class="form-horizontal" id="startexamform"
          action="" method="post">
        <div id="content">
            <center>
...
<script>
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    if (data[i].titleTypeName != strmulu) {
                        console.log("in:" + data[i].titleTypeName);
                        onesum_mulu += 1;
                        strmulu = data[i].titleTypeName;
    ...
    function submitsave() {
        if (confirm("您确定要提交并关闭吗？")) {
            $.ajax({
                dataType: "json",
                url: "/school/onlineExam/userTest/setState",
                data: {
                    'paperId': paperId,
                    'studentId': studentId,
                    'tutId': tutId
    ...
    function anwserchange(questionnum, questionid, anwserid) {
        ...
        if (questionnum == 1) {
            ...
            let result = $("input[title='" + questionid + "']").val();
            if (result == 'F') {
            ...
            let edStuanwser = document.getElementById(anwserid).value;
    ...
    (function () {
        ...
        let wsUrl = "ws://127.0.0.1:8089/wsk";
        let tt;
