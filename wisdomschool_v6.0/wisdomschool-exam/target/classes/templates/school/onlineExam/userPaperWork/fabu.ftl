...
<div class="container-div layout-container-div clearfix">
    <form class="form-horizontal m" id="form-userPaperWork-add">
        <!--学生树-->
...
<script type="text/javascript">
    ...
    var tid = "${tid}";
    var stuUserId = "";
    var prefix = "/school/onlineExam/userPaperWork";
    var setting = {
        view: {
            selectedMulti: false,
        },
    ...
    (function () {
        ...
        $.ajax({
            type: 'POST',
            url: '/school/onlineExam/userPaperWork/getStudents?cId=' + cId + "&&tid=" + tid,
            dataType: 'json',
...
//年级
    ...
    function getTcoName() {
        $.ajax({
            type: 'POST',
            url: '/school/onlineExam/userPaperWork/getTcoName?cId=' + cId + "&&tid=" + tid,
            dataType: 'json',
            success: function (data) {
                for (let i = 0; i < data.length; i++) {
                    let datavalue = {};
...
//获取学生
    function getTcoStu() {
        $.ajax({
            type: 'POST',
            url: '/school/onlineExam/userPaperWork/getTcoStu?cId=' + cId + "&&tid=" + tid,
            dataType: 'json',
            success: function (data) {
                for (let i = 0; i < data.length; i++) {
                    let datavalue = {};
...
<script>
    (function () {
        ...
        if(rule=="1"){
            console.log("11111");
            $('#form-userPaperWork-add').empty();
            $('#form-userPaperWork-add').append("<div>" +
