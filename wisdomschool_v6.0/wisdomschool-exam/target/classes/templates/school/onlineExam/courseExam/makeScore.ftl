    ...
    <div class="col-sm-12 select-table table-striped" style="width: 50%;left:25%" id="piyue">
        <div class="form-horizontal" id="startexamform">
            <div id="content">
                ...
                <button class="btn btn-info ipt" type="button"
                        onClick="doSum()">
                    <i class="ace-icon fa fa-check bigger-110"></i>
                    计算分数
                </button>
...
<script>
    ...
    function initPaper() {
        $.ajax({
                type: "post",
                dataType: 'json',
                url: "/school/onlineExam/courseExam/startMakePaper?paperId=" + paperId + "&&studentId=" + studentId,
                            ...
                            for (let j = 0; j < ansList.length; j++) {
                                ...
                                if (stuAnsList.length != 0) {
                                    if ((stuAnsList[j].stuanswer == '0') && (ansList[j].stanswer != '0')) {
                                        trueAns = 'red';
                                    } else if ((stuAnsList[j].stuanswer != '0') && (ansList[j].stanswer == '0')) {
                                        stutrueAns = '';
                                    } else if (stuAnsList[j].stuanswer != '0') {
                                        isCheck = 'checked';
                                    }
    ...
    function submitsave() {
        if (confirm("您确定要提交并关闭吗？")) {
            $.ajax({
                type: "post",
                dataType: 'json',
                url: "/school/onlineExam/courseExam/saveScore?paperId=" + paperId + "&studentId=" + studentId,
                error: function (data) {
                    document.write("提交成功！");
                },
                success: function (data) {
    ...
    function doSum() {
        $.ajax({
            type: "post",
            dataType: 'json',
            url: "/school/onlineExam/courseExam/doSum?paperId=" + paperId + "&studentId=" + studentId,
            error: function (data) {
            ...
            success: function (data) {
                for (var i = 0; i <data.length; i++) {
                    $('#fenshu').empty();
                    $('#fenshu').append("分数："+data[i].stuScore);
                }
            }
        });
    }
    ...
    function anwserchange(questionnum, questionid, questionscore, qscore) {
        ...
        $.ajax({
            type: "post",
            url: "/school/onlineExam/courseExam/saveStuScore?studentId=" + studentId + "&&questionid=" + questionid + "&&questionscore=" + score+"&&paperId"+paperId,
            error: function (data) {
