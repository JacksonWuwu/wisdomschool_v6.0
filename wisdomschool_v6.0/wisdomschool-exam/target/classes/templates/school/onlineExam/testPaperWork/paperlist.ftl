        ...
        let options = {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    align: 'center',
                    formatter:function (value, row, index) {
                        let actions = row.startTime+'</br>'+row.endTime
...
<#--                        </@shiro.hasPermission>-->
                        <#--                        <@shiro.hasPermission name="teacher:course:view">-->
                        <#--                        actions.push('<a class="btn btn-success btn-xs " href="#" onclick="addExam(\'' + row.id + '\')"><i class="fa fa-edit"></i>发布</a> ');-->
                        <#--                        </@shiro.hasPermission>-->
<#--                        <@shiro.hasPermission name="teacher:course:view">-->
...
<#--                        </@shiro.hasPermission>-->
                        ...
                        <#--                        </@shiro.hasPermission>-->
<#--                        <@shiro.hasPermission name="teacher:course:view">-->
                        actions.push('<a class="btn btn-success btn-xs " href="#" onclick="setExamY(\'' + row.id + '\')"><i class="fa fa-edit"></i>开始  </a> ');
<#--                        </@shiro.hasPermission>-->
<#--                        <@shiro.hasPermission name="teacher:course:view">-->
...
<#--                        </@shiro.hasPermission>-->
    ...
    function AGAbuild() {
        ...
        $.modal.openFull('智能组卷', "/school/onlineExam/testPaperWork/AGAbuild?id=" + id+"&&cid="+cid, saveCallback);

    }
    function publish(id) {
        let cid = "${id}";
        let result = $.modal.open('发布', "/school/onlineExam/testPaperWork/buileTest?id="+id, function () {
    ...
    function Humanbuild(tid) {
        ...
        var prule = paper[1];
        if (prule == "随机") {
            $.modal.openFull('随机组卷', "/school/onlineExam/testPaperWork/Coursebuild?id=" + pid, saveCallback);
        }else{
    ...
    function detail(id) {
        $.modal.openFull( '详情', '/school/onlineExam/testPaperWork/showDetail?id=' + id, saveCallback);
    }
    function examControl(id) {
        $.modal.open( '监考链接', '/school/onlineExam/testPaperWork/examControl?id=' + id,saveCallback,'800px','300px');
    }
    function addExam(id) {
    ...
    {
        window.open('/school/onlineExam/testPaperWork/stuToTest?id='+id,'_blank');
    ...
    function analyze(id) {
        var cid = "${id}";
        $.modal.open('作业分析', "/school/onlineExam/testPaperWork/analyzePaper?id=" + id+"&&cid="+cid, saveCallback);
    }
    ...
    function Coursebuild(tid) {
        ...
        if (prule == "随机") {
            ...
            $.modal.openFull('固定组卷', "/school/onlineExam/testPaperWork/Humanbuild?id=" + pid+"&&cid="+cid, saveCallback);
        }
    }
    function setExamY(id) {
        ...
        $.modal.confirm("确认要开始?", function () {
            ...
            $.operate.submit("/school/onlineExam/testPaperWork/setExamY", "post", "json", data, true, function (result) {
                if (result.code === web_status.SUCCESS) {
    ...
    function setExamN(id) {
        ...
        $.modal.confirm("确认要取消?", function () {
            ...
            $.operate.submit("/school/onlineExam/testPaperWork/setExamN", "post", "json", data, true, function (result) {
                if (result.code === web_status.SUCCESS) {
                    $.modal.alertSuccess("操作成功！");
                } else {
