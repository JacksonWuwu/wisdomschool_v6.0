 ...
 */
@Controller
@RequestMapping("/school/onlineExam/testPaper")
public class TestPaperController extends BaseController {
    private String prefix = "school/onlineExam/testPaper";
    /**
     * 科目ID
     */
    ...
    @RequestMapping(value = "/getTestPaperById/{testPaperId}")
    @ResponseBody
    TestPaper getTestPaperById(@PathVariable("testPaperId") String testPaperId){
        return testPaperService.getById(testPaperId);
    }

    @GetMapping("/{cid}")
    public String toList(@PathVariable("cid")String cid, ModelMap modelMap) {
        modelMap.put("id", cid);
        return prefix + "/list";
    }
...
//        }
    ...
    @ResponseBody
    public TableDataInfo homeworkList(@PathVariable String cid, TestPaper testPaper) {
        System.out.println("@#####");
        startPage();
        testPaper.setCoursrId(cid);
        testPaper.setType("1");
    ...
    public Data add(TestPaper testPaper) throws Exception {
        ...
        testPaper.setType("0");
        testPaper.setCreateId(getUser().getUserAttrId());
        return toAjax(testPaperService.save(testPaper)); //TestPaperMapper.xml->insert  涉及数据表：tk_test_paper
    ...
    @ResponseBody
    public Data addhomework(TestPaper testPaper) throws Exception {
        System.out.println(testPaper.getCoursrId());
        testPaper.setCreateBy(getUser().getLoginName());
        testPaper.setType("1");
        testPaper.setCreateId(getUser().getUserAttrId());
    ...
    @PostMapping("/edit")
    @ResponseBody
    public Data edithomework(TestPaper testPaper) throws Exception {
    ...
    @ResponseBody
    public Data homeworkedit(TestPaper testPaper) throws Exception {
        testPaper.setUpdateId(getUser().getUserAttrId());
        testPaper.setUpdateBy(getUser().getLoginName());
        return toAjax(testPaperService.update(testPaper));
    }
    ...
    @ResponseBody
    public String checkHeadlineUnique(TestPaper testPaper) {
        return testPaperService.checkTestNameUnique(testPaper);
    }
    ...
    public List<QuestionPaper> AGAbuildTestStepTwo(ModelMap mmap) {
        ...
        for (MyQuestions myQuestions : AGAqList) {
            QuestionPaper up = new QuestionPaper();
            List<String> tarr = new ArrayList<String>();
            List<String> tList = new ArrayList<String>();
            up.setTid(myQuestions.getId());
            up.setTitle(myQuestions.getTitle());
            up.setTitleType(myQuestions.getTitleTypeName());
            up.setNumber(myQuestions.getTitleTypeId());
            up.setDifficulty(myQuestions.getDifficulty());
            up.setQexposed(myQuestions.getQexposed());
            up.setScore(myQuestions.getQuestionscore());
    ...
    public Data humanAdd(@RequestParam String testQuestionsId, @RequestParam String testPaperId) throws
        ...
        List<TestpaperQuestions> testQuestionsList = new ArrayList<TestpaperQuestions>();
        int testpaperScore = 0;
        for (String s : tidarry) {
            String[] strarr = s.substring(0, s.length() - 1).split(";");
            String id = strarr[0];
            MyQuestions myq = this.myQuestionsService.getById(id);
            int questionExposed = Integer.parseInt(myq.getQexposed());
            questionExposed += 1;
            ...
            if (myoastr != null && !"".equals(myoastr)) {
                String[] myoaArr = myoastr.substring(0, myoastr.length() - 1).split(";");
                List<MyOptionAnswer> myoalist = new ArrayList<MyOptionAnswer>();
                for (String value : myoaArr) {
                    MyOptionAnswer myOptionAnswer = myOptionAnswerService.getById(value);
                    myoalist.add(myOptionAnswer);
                }
        ...
        TestPaper testPaper = testPaperService.getById(testid);
        //  一对多关系映射
        for (TestpaperQuestions testpaperQuestions : testQuestionsList) {
            TestpaperTestquestions testPaperTestQuestions = new TestpaperTestquestions();
            testPaperTestQuestions.setTestQuestionsId(testpaperQuestions.getId());
            testPaperTestQuestions.setTestPaperId(testPaper.getId());
            Uuid uuid = new Uuid();
            testPaperTestQuestions.setId(uuid.UId());
            testPaperTestQuestions.setCreateId(uid);
    ...
    @ResponseBody
    public List<TestPaper> getStudents() {
        return testPaperService.getStudentInfo("37");
    ...
    @ResponseBody
    public List<TestPaper> getTcoName() {
        return testPaperService.getTcoName("37");
    ...
    @ResponseBody
    public List<TestPaper> getTcoStu() {
        return testPaperService.getTcoStu("37");
...
//                }
    ...
    public TableDataInfo list(@PathVariable String id) throws Exception {
        ...
        List list = userTestService.selectList(userTest);
        return wrapTable(list);
    }
    ...
    public List<TestpaperQuestions> startMakePaper(String paperId, String studentId, ModelMap mmap) throws Exception {
        List<TestpaperQuestions> tqvolist = testpaperTestquestionsService.getQuestionsAndOptionsByPaperIdWithUserId(paperId, getUser().getId());
        tqvolist.sort((x, y) -> Double.compare(Double.parseDouble(x.getTitleTypeNum()), Double.parseDouble(y.getTitleTypeNum())));//这方法需要jdk1.8以上
        return tqvolist;
    }
    ...
    @ResponseBody
    public Data saveScore(String paperId, String userId, @RequestBody List<OptionSubmitVo> scoreArray) throws Exception {
        List<StuOptionanswer> optionanswers = new ArrayList<>();
        scoreArray.forEach(o -> {
            StuOptionanswer stuOptionanswer = new StuOptionanswer();
            stuOptionanswer.setId(o.getStuOptionanswerId());
            stuOptionanswer.setQuestionScore(o.getScore());
            stuOptionanswer.setUpdateId(Integer.valueOf(getUserId()));
            optionanswers.add(stuOptionanswer);
        });
        return toAjax(stuOptionanswerService.updateListAndTotalScore(optionanswers, paperId, userId));
    }
    ...
    public static void setTestPaperName(String testPaperName) {
        TestPaperController.testPaperName = testPaperName;
    }
    ...
    public static void setqList(List<MyQuestions> qList) {
        TestPaperController.qList = qList;
    }
    ...
    public static void setqList_chapter(List<MyQuestions> qList_chapter) {
        TestPaperController.qList_chapter = qList_chapter;
    }
    ...
    public static void setqList_course(List<MyQuestions> qList_course) {
        TestPaperController.qList_course = qList_course;
    }
    ...
    public static void setLists(List<MyQuestions> lists) {
        TestPaperController.lists = lists;
    }
    ...
    public static void setTitle_name(String title_name) {
        TestPaperController.title_name = title_name;
    }
    ...
    public static void setAGAqList(List<MyQuestions> AGAqList) {
        TestPaperController.AGAqList = AGAqList;
    }
    ...
    public static void setQuestionsid(String[] questionsid) {
        TestPaperController.questionsid = questionsid;
    }
    ...
    public static void setNameL(ArrayList nameL) {
        TestPaperController.nameL = nameL;
    }
}
