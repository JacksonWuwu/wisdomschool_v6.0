...
<script>
    ...
    function initPaper() {
        $.ajax({
            type: "post",
            dataType: 'json',
            url: "/school/onlineExam/testPaper/startMakePaper?paperId=" + paperId + "&&studentId=" + studentId,
            ...
            success: function (data) {
                ...
                var numitem = 0;
                var onesum_mulu = 0;
                stuOptionanswerIdArray = new Array();
                for (let i = 0; i < data.length; i++) {
                    initStuOptionId(data[i].stuOptionanswerList[0].id);
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    if (data[i].tQuestiontemplateNum == "3") {
                        ...
                        for (let j = 0; j < ansList.length; j++) {
                            ...
                            if (stuAnsList != null) {
                                stuScore3 = stuAnsList[j].questionScore;
                            } else {
                                stuScore3 = "0";
                            }
                            let fla = String.fromCharCode(flag);
                    ...
                    if (data[i].tQuestiontemplateNum == "4") {
                        ...
                        for (let j = 0; j < ansList.length; j++) {
                            ...
                            var stuGetScore = "";
                            if (stuAnsList != null) {
                                stuGetScore = stuAnsList[j].questionScore;
                            }
                    ...
                    if (data[i].tQuestiontemplateNum == "5") {
                        ...
                        for (let j = 0; j < ansList.length; j++) {
                            ...
                            if (stuAnsList != null) {
                                stuScore5 = stuAnsList[j].questionScore;
                            } else {
                                stuScore5 = "0";
                            }
                            $('#content').append(' <li style="list-style: none">' +
                    ...
                    if (data[i].tQuestiontemplateNum == "6") {
                        ...
                        for (var j = 0; j < ansList.length; j++) {
                            ...
                            if (stuAnsList != null) {
                                stuScore6 = stuAnsList[j].questionScore;
                            } else {
                                stuScore6 = "0";
                            }
                            $('#content').append(' <li style="list-style: none">' +
    ...
    function submitsave() {
        ...
        if (stuOptionanswerIdArray.length != 0) {
            for (var i=0; i<stuOptionanswerIdArray.length; i++){
                var object = {
                    'stuOptionanswerId': stuOptionanswerIdArray[i],
                    'score': $("input[name='" + stuOptionanswerIdArray[i] +"']").val()
                }
        ...
        if (confirm("您确定要提交并关闭吗？")) {
            $.ajax({
                type: "post",
                dataType: 'json',
                url: "/school/onlineExam/testPaper/saveScore?paperId=" + paperId + "&userId=" + "${stuUserId}",
    ...
    function testAnswer(questionnum, questionid, anwserid, anwserstr, stuoptionid, index) {
        ...
        if (!questionidArray.includes(questionid)) {
            ...
            stuOptionanswerIdArray[index] = stuoptionid;
            tqg++;
        } else {
    ...
    function initStuOptionId(optionId) {
        stuOptionanswerIdArray.push(optionId);
    }
