...
<script type="text/javascript">
    ...
    function submitHandler() {
        ...
        if (stuId == "") {
            ...
            $.operate.saveModal(prefix + "/addQ", $.param({
                "cId": cId,
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $.ajax({
            type: 'POST',
            url: '/school/onlineExam/course/getQStudents?cId=' + cId + "&&tid=" + tid,
            dataType: 'json',
...
//班级
    function getTcoName() {
        $.ajax({
            type: 'POST',
            url: '/school/onlineExam/course/getQTcoName?cId=' + cId + "&&tid=" + tid,
            dataType: 'json',
            success: function (data) {
                for (let i = 0; i < data.length; i++) {
                    let datavalue = {};
...
//学生
    function getTcoStu() {
        $.ajax({
            type: 'POST',
            url: '/school/onlineExam/course/getQTcoStu?cId=' + cId + "&&tid=" + tid,
            dataType: 'json',
            success: function (data) {
                console.log("getQTcoStu");
                for (let i = 0; i < data.length; i++) {
                    console.log("hello");
                    let datavalue = {};
