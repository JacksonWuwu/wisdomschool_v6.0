package cn.wstom.storage.filesystemmanager;
 ...
 * （可能会有一定的耗时）。
 * 每次使用前都请手动调用cn.wstom.storage.server.util.FileNodeUtil.initNodeTableToDataBase()方法以确保节点链接最新。
 * </p>
 *
 * @author dws
 * @version 1.0
...
public class FileSystemManager {
    ...
    private FileSystemManager() {
        ...
        try {
            ...
            LOGGER.error("错误：出现未知错误，文件系统解析失败，无法浏览文件。");
        }
    }
     ...
     * </p>
     *
     * @return kohgylw..util.file_system_manager.FileSystemManager 唯一实例
     */
     ...
     * @return kohgylw..util.file_system_manager.pojo.FolderView 指定的文件夹视图
    ...
    public boolean delete(String[] foldersId, String[] filesId) throws SQLException {
        undo = true;
        for (int i = 0; i < filesId.length && undo; i++) {
            deleteFile(filesId[i]);
        }
        for (int i = 0; i < foldersId.length && undo; i++) {
    ...
    public boolean exportTo(String[] foldersId, String[] nodesId, File path, String type) throws Exception {
        undo = true;
        for (int i = 0; i < nodesId.length && undo; i++) {
    ...
    public boolean importFrom(File[] files, String folderId, String type) throws Exception {
        undo = true;
        for (int i = 0; i < files.length && undo; i++) {
     ...
     * @return java.util.List<kohgylw..server.model.Folder> 文件夹对象列表，如果没有结果则长度为0
    ...
    private void importFileInto(File f, String folderId, String type) throws Exception {
        if (f.isFile()) {
            ...
            int length = 0;
            long finishLength = 0;
            while ((length = in.read(buffer)) != -1 && undo) {
                buffer.flip();
                out.write(buffer);
                buffer.clear();
                finishLength += length;
    ...
    private void importFolderInto(File f, String folderId, String type) throws Exception {
        if (f.isDirectory()) {
            ...
            for (int i = 0; i < childs.length && undo; i++) {
    ...
    private void exportNode(String nodeId, File path, String type) throws Exception {
        ...
        if (node != null && path != null && path.isDirectory()) {
            ...
            int length = 0;
            long finishLength = 0;
            while ((length = fci.read(buffer)) != -1 && undo) {
                buffer.flip();
                fco.write(buffer);
                buffer.clear();
                finishLength += length;
    ...
    private void exportFolder(String folderId, File path, String type) throws Exception {
        ...
        if (folder != null && path != null && path.isDirectory()) {
            ...
            int size = 0;
            int i = 0;
            for (i = 0, size = nodes.size(); i < size && undo; i++) {
                exportNode(nodes.get(i).getFileId(), target, type);
            }
            for (i = 0, size = folders.size(); i < size && undo; i++) {
