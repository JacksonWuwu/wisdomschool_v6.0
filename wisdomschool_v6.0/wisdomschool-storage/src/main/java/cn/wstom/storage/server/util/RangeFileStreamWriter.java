...
public class RangeFileStreamWriter {
    ...
    protected void writeRangeFileStream(HttpServletRequest request, HttpServletResponse response, File fo, String fname,
        ...
        if (request.getHeader("User-Agent").toLowerCase().contains("safari")) {
        ...
        if (startOffset != 0) {
            ...
            String contentRange;
            if (!hasEnd) {
                contentRange = "bytes " + startOffset + "-" +
                        (fileLength - 1) + "/" + fileLength;
                response.setHeader("Content-Range", contentRange);
            } else {
                contentRange = "bytes " + rangeBytes + "/" + "" + fileLength;
            }
            response.setHeader("Content-Range", contentRange);
        }
        // 写出缓冲
