...
import cn.wstom.gateway.entity.SysUser;
...
public class JwtGateWayUtils {
	...
	public String generateToken(SysUser sysUser) {
		...
		claims.put(JwtConstant.CLAIM_KEY_USERID, sysUser.getId());
		claims.put(JwtConstant.CLAIM_KEY_USERNAME, sysUser.getLoginName());
	...
	public Date getExpiredDateFromToken(String token) {
		...
		log.info("token中过期时间 {}", new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(expiredDate));
		return expiredDate;
	}
	...
	public boolean validateToken(String token, SysUser sysUser) {
		...
		return username.equals(sysUser.getLoginName()) && !isTokenExpired(token);
	}
